...
export function calculateTimeToHire(applications: Application[]): TimeToHireMetrics {
    ...
    const daysToHire = hiredApps.map(app =>
        differenceInDays(new Date(app.updatedAt), new Date(app.appliedDate))
    );
    ...
    hiredApps.forEach(app => {
        const days = differenceInDays(new Date(app.updatedAt), new Date(app.appliedDate));
        ...
        const avgTimeToHire = hired.length > 0
            ? Math.round(
                hired.reduce((sum, app) =>
                    sum + differenceInDays(new Date(app.updatedAt), new Date(app.appliedDate)), 0
                ) / hired.length
            )
            : 0;
