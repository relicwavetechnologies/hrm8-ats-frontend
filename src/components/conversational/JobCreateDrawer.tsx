...
import { BriefcaseBusiness, X, ChevronRight, Loader2, CheckCircle2 } from 'lucide-react';
    ...
    return (
        <>
            <Drawer open={open} onOpenChange={(nextOpen) => { if (!nextOpen) handleRequestClose(); else onOpenChange(nextOpen); }}>
                <DrawerContent className="h-[94vh] rounded-t-2xl border bg-background shadow-xl flex flex-col overflow-hidden">
                    <DrawerHeader className="border-b px-4 py-3 bg-background/95 backdrop-blur-md sticky top-0 z-10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-md bg-muted flex items-center justify-center">
                                <BriefcaseBusiness className="h-4 w-4 text-foreground" />
                            </div>
                            <div>
                                <DrawerTitle className="text-base font-semibold tracking-tight">Smart Job Wizard</DrawerTitle>
                                <DrawerDescription className="text-xs text-muted-foreground">
                                    Step {WIZARD_STEPS.indexOf(currentStepId) + 1} of {WIZARD_STEPS.length} • {stepTitles[currentStepId] || currentStepId}
                                </DrawerDescription>
                            </div>
                        </div>
                        <Button variant="ghost" size="icon" onClick={handleRequestClose} className="h-8 w-8 rounded-md hover:bg-muted">
                            <X className="h-4 w-4" />
                        </Button>
                    </DrawerHeader>
                    ...
                    <div className="border-b px-4 py-2 bg-muted/10">
                        <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                            <div
                                ...
                                style={{ width: `${((WIZARD_STEPS.indexOf(currentStepId) + 1) / WIZARD_STEPS.length) * 100}%` }}
                            />
                        </div>
                            ...
                            {WIZARD_STEPS.map((stepId, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-[11px] whitespace-nowrap transition-colors",
                                            isActive && "bg-primary text-primary-foreground border-primary",
                                            !isActive && "bg-background text-muted-foreground hover:text-foreground",
                                        )}
                                    >
                                        {isDone ? <CheckCircle2 className="h-3 w-3" /> : <span className="text-[10px] font-semibold">{idx + 1}</span>}
                    ...
                    <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col min-w-0 bg-muted/20">
                            ...
                            {currentStepId !== 'document-upload' && (
                                <div className="px-4 py-2.5 bg-background border-t flex justify-between items-center">
                                    <Button variant="ghost" size="sm" onClick={prevStep} className="gap-2 h-8">
                        ...
                        <div className="w-[420px] border-l bg-white hidden lg:flex flex-col min-h-0">
                            <div className="px-4 py-2.5 border-b bg-muted/10 flex items-center justify-between">
                                <h3 className="font-semibold text-xs uppercase tracking-wide text-muted-foreground">Live Job Preview</h3>
                                <div className="px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px] font-bold">LIVE</div>
                            </div>
